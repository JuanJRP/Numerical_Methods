...
# Aplicando la fórmula de los mínimos cuadrados
x1_mean = np.mean(X1)
x2_mean = np.mean(X2)
y_mean = np.mean(Y)
...
display_step = 100 #Cada cuantas iteraciones deseamos ver los resultados
N = len(X1)
loss = []
...
for step in range(epochs+1):
    ...
    for i in range(N):
        ...
        m_gradient = (2/N) * (Diferencia) *                 np.sum((X1[i] - x1_mean) * (Y[i] - y_mean))
        b_gradient = (2/N) * (Diferencia) *                 np.sum((X2[i] - x2_mean) * (Y[i] - y_mean))
    ...
    if max(abs(learning_rate * m_gradient), abs(learning_rate * b_gradient)) <= convergence_criteria and MSE < 1:
        break
